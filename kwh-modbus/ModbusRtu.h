...
{
private:
	TSerial * port; //!< Pointer to Serial class object
...
public:
	...
	* @ingroup setup
	* @overload Modbus(uint8_t u8id, uint8_t u8serno)
	* @overload Modbus(uint8_t u8id)
	* @overload Modbus()
	*/
	Modbus(TSerial *serial, uint8_t u8id)
	{
		port = serial;
		init(u8id, 0);
	}
	...
	* @ingroup setup
	* @overload Modbus(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
	* @overload Modbus(uint8_t u8id)
	* @overload Modbus()
	*/
	Modbus(TSerial *serial, uint8_t u8id, uint8_t u8txenpin)
	{
		port = serial;
		init(u8id, u8txenpin);
	}
	...
	* using "void begin(SoftwareSerial *softPort, long u32speed)".
	*
	* @param u8id   node address 0=master, 1..247=slave
	* @ingroup setup
	* @overload Modbus(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
	...
	* Call once class has been instantiated, typically within setup().
	*
	* @overload begin(uint16_t u16BaudRate)
	* @ingroup setup
	*/
	void begin()
	{
